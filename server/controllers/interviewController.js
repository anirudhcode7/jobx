...
const evaluateInterview = async (req, res) => {
  try {
    ...
    const latestInterview = interviewData.interviews.reduce(
      (latest, current) => {
        return latest.attempt_number > current.attempt_number
          ? latest
          : current;
      }
    );
    ...
    const messages = [
      {
        role: "system",
        content:
          "Evaluate the interview on communication skills, relevancy, and expertise with scores from 0 to 5 (0.5 increments). Provide concise feedback for each question-answer pair. Output in JSON format.\n",
      },
      {
        role: "user",
        content:
